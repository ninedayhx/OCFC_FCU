...
void MX_ADC1_Init(void)
{
  ...
  */
  ...
  //__HAL_DMA_ENABLE_IT(&hdma_adc1, DMA_IT_TC); // 使能DMA1adc1传输完成中断
...
void MX_ADC2_Init(void)
{
  ...
  */
  ...
  //__HAL_DMA_ENABLE_IT(&hdma_adc2, DMA_IT_TC); // 使能DMA1adc2传输完成中断
...
void HAL_ADC_MspInit(ADC_HandleTypeDef* adcHandle)
{
  ...
  */
  ...
  /* USER CODE BEGIN ADC1_MspInit 1 */
	__HAL_DMA_ENABLE_IT(&hdma_adc1, DMA_IT_TC); // 使能DMA1传输完成中断
  ...
  /* USER CODE BEGIN ADC2_MspInit 0 */
	__HAL_DMA_ENABLE_IT(&hdma_adc2, DMA_IT_TC); // 使能DMA2传输完成中断
...
void read_all_adc2_values(uint16_t* adc_values, uint8_t num_channels){
	HAL_ADC_Start_DMA(&hadc2, adc_values, num_channels);
}
...
float convert_adc_to_voltage(uint16_t adc_value, float voltage_ref, float voltage_divider_ratio) {
    float voltage = adc_value * voltage_ref / 4095.0f / voltage_divider_ratio;
    return voltage;
}
//分流器的AD值转电流(单位A)
float convert_adc_to_current(uint16_t adc_value, float voltage_ref, float shunt_max_current){
	//float current = (adc_value * (shunt_max_current / 4095.0f ));
	float current = ((adc_value * (voltage_ref / 4095.0f)) * (shunt_max_current / voltage_ref ));
	return current;
}
//瞬时功率计算(输入电压V,电流A,输出kW)
